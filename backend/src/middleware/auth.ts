...
// ---------------------------------------------------------------------------
const DEV_BYPASS = env.nodeEnv !== 'production' && process.env.DEV_BYPASS_AUTH === 'true';
...
if (DEV_BYPASS) {
    ...
    if (!DEV_USER_ID) {
        console.warn('⚠️  [auth] DEV_USER_ID is not set — set it to a valid public.users UUID.\n');
    }
}
