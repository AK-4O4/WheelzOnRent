...
export const kycDocumentTypeEnum = pgEnum('kyc_document_type', ['cnic_front', 'cnic_back', 'license', 'selfie']);
export const vehicleDocumentTypeEnum = pgEnum('vehicle_document_type', ['registration', 'insurance']);
export const vehicleStatusEnum = pgEnum('vehicle_status', ['active', 'under_review', 'suspended']);
export const bookingStatusEnum = pgEnum('booking_status', ['pending', 'confirmed', 'ongoing', 'completed', 'cancelled']);
export const paymentTypeEnum = pgEnum('payment_type', ['deposit', 'rental_fee', 'refund']);
export const paymentStatusEnum = pgEnum('payment_status', ['pending', 'completed', 'failed', 'refunded']);
export const disputeStatusEnum = pgEnum('dispute_status', ['open', 'under_review', 'resolved', 'rejected']);
export const adminLevelEnum = pgEnum('admin_level', ['super_admin', 'moderator', 'support']);
...
export const userKycDocuments = pgTable('user_kyc_documents', {
    ...
    reviewedBy: uuid('reviewed_by').references(() => admins.userId, { onDelete: 'set null' }),
...
export const disputes = pgTable('disputes', {
    id: uuid('id').defaultRandom().primaryKey(),
    bookingId: uuid('booking_id').references(() => bookings.id).unique().notNull(), // 1:1
...
export const reviews = pgTable('reviews', {
    ...
    oneReviewPerBooking: uniqueIndex('one_review_per_booking').on(table.bookingId, table.reviewerId),
}));

export const adminLogs = pgTable('admin_logs', {
    id: uuid('id').defaultRandom().primaryKey(),
    adminId: uuid('admin_id').references(() => admins.userId, { onDelete: 'cascade' }).notNull(),
...
export const conversations = pgTable('conversations', {
    ...
    uniqueConversation: uniqueIndex('unique_conversation').on(table.renterId, table.ownerId, table.vehicleId),
}));
